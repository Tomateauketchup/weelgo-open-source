...
public abstract class UpdateListProcessor<T>
{
	...
	public abstract void setUuid(T o,
			String uuid);

	public abstract boolean isEqual(T oldElem,
			T newElem);
	...
	public void update(List<T> oldTagsArray,
			List<T> newTagsArray)
	{
			...
			for (T oldElem : oldTagsArray)
			{
				if (oldElem != null)
				{
					...
					if (newTagsArray != null)
					{
						...
						{
							if (newElem != null && CoreUtils.isStrictlyEqualsString(getUuid(oldElem), getUuid(newElem)))
							{
								remove = false;
